...
setkey(df, ContactID, ActivityDate)
df[, Sales.Presentation_F := funfuture(ActivityType == "Sales.Presentation" , ActivityDate, 35), by = ContactID]
df[, Tickets_F := funfuture(ActivityType == "Sale" & SalesAmount>0 , ActivityDate, 35), by = ContactID]
df[, Meeting_F := funfuture(ActivityType == "Meeting" , ActivityDate, 35), by = ContactID]



setkey(df4, ContactID, ActivityDate,ActivityType)
setkey(df, ContactID,ActivityDate, ActivityType)


df5<-df[df4][,c(52,53,1,2,3,4,48,49,50,51),with=FALSE]
